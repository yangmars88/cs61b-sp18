...
public class ArrayDeque<T> implements Deque<T> {
    ...
    public void addFirst(T item){
     ...
     private int minusOne(int index) {
            if (index == 0) {
                return capacity - 1;  //only when inserting from front end, the nextFirst will be decreased,
                                     // so when nextFirst is 0, means, array has been fully inserted from capacity-1 to 0, so the first item pointer should go back to the very first item.
            } else {
                return index - 1;  //if index not 0, means there is still empty space for inserting,
            }
      }
    @Override
    ...
    public T get(int index) {
        ...
        return items[indexFromFront];  //if insert from front end, this will always return items[capacity-1]
    }
